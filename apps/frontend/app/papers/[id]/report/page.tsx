...
import {
  getReport, reviewFinding, downloadReportPdf,
  ReportResponse, FindingResponse, NetworkError, ApiError,
} from "@/lib/api";
      ...
      {finding.requires_human_review && finding.status === "draft" && (
        <div className="border-t border-slate-100 pt-3 space-y-2">
          <p className="text-xs text-slate-500">
            This finding requires expert review before it can be shared.
          </p>
          <textarea
...
function ReportContent({ paperId }: { paperId: string }) {
  ...
  useEffect(() => {
    if (!reportId) { setError("No report ID in URL."); return; }
    let stopped = false;
    async function poll() {
      while (!stopped) {
        try {
  ...
  if (!report || report.status === "pending" || report.status === "partial") return (
    <div className="max-w-3xl mx-auto space-y-4">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-8 text-center space-y-3">
        ...
        <p className="text-sm text-slate-600">Agents are running…</p>
        {report?.status === "partial" && (
          <p className="text-xs text-slate-400">Some agents finished. Waiting for the rest…</p>
        )}
      </div>
    </div>
  );
  ...
  const agents = Array.from(new Set(report.findings.map((f) => f.agent)));
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6 space-y-3">
        <div className="flex items-center justify-between flex-wrap gap-2">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button
              ...
              className="px-3 py-1.5 bg-slate-800 text-white text-xs font-medium rounded-lg hover:bg-slate-700 disabled:opacity-50 flex items-center gap-1.5 transition-colors"
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="w-3.5 h-3.5" viewBox="0 0 20 20" fill="currentColor">
        ...
        <div className="flex flex-wrap gap-3 text-xs text-slate-400">
          ...
          {agents.map((a) => (
            <span key={a} className="capitalize">{a.replace(/_/g, " ")}</span>
          ))}
          {report.completed_at && (
        ...
        <div className="flex gap-2 flex-wrap text-xs">
          {(["high", "medium", "low", "info"] as const).map((s) => {
            ...
            return (
              <span key={s} className={`px-2 py-0.5 rounded font-medium ${SEVERITY_STYLES[s].badge}`}>
                {count} {s}
              </span>
            );
          })}
        ...
        <div className="space-y-3">
          ...
          {(["high", "medium", "low", "info"] as const).flatMap((severity) =>
            report.findings
              ...
              .map((f) => <FindingCard key={f.id} finding={f} onReview={handleReview} />)
          )}
        </div>
      ) : (
...
export default function ReportPage({ params }: { params: { id: string } }) {
  return (
    <Suspense fallback={
      ...
      <ReportContent paperId={params.id} />
    </Suspense>
  );
}
