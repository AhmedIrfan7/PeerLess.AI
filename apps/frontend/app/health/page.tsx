    ...
    try {
      ...
      setComponents([
        ...
        { label: "Cache (Redis)", status: redisOk ? "ok" : "error", detail: redisOk ? "connected" : h.redis },
        {
          ...
          detail: llmOk ? "API key present — Gemini available" : "No API key — regex fallback active",
        },
      ]);
  ...
  return (
    <div className="max-w-xl mx-auto space-y-5">
      ...
      <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 text-xs text-slate-500 space-y-1">
        ...
        <p className="mt-2">The <strong>LLM</strong> row shows orange if no Gemini key is set. Statistical checks (GRIM + p-value) still run automatically via regex — the system is fully functional without a key.</p>
      </div>
